...
const VerifyEmailPage = ({ setCurrentPage }) => {
  ...
  const handleVerify = async () => {
    ...
    setLoading(true);
    try {
      await AuthService.verifyResetCode(fullCode);
      setShowSuccess(true);
      setTimeout(() => setCurrentPage('newpassword'), 2000);
    } catch (err) {
    ...
    <div style={styles.wrapper}>
      ...
      <div style={styles.card}>
          ...
          <div style={styles.inputRow}>
            {code.map((digit, index) => (
              <input
                ...
                style={{ ...styles.codeInput, borderColor: digit ? '#155DFC' : '#e5e7eb', color: '#1a1a1a', background: '#f9fafb' }}
              />
            ))}
          </div>
        </div>
        ...
        <button
          ...
          style={{ ...styles.verifyBtn, opacity: code.join('').length !== 6 || loading ? 0.7 : 1, cursor: code.join('').length !== 6 || loading ? 'not-allowed' : 'pointer' }}
        >
      ...
      {showSuccess && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.35)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: "40px 48px", textAlign: "center", boxShadow: "0 8px 32px rgba(0,0,0,0.15)", minWidth: 320 }}>
            ...
            <h5 style={{ fontWeight: 700, color: "#1a1a1a", marginBottom: 8, fontSize: 20 }}>Verified Successfully</h5>
            <p style={{ color: "#aaa", fontSize: 14, margin: 0 }}>Your account has been verified</p>
            <div style={{ marginTop: 20, height: 4, background: "#e5e7eb", borderRadius: 2, overflow: "hidden" }}>
              <div style={{ height: "100%", background: "#22c55e", borderRadius: 2, animation: "vprogress 2s linear forwards" }} />
            </div>
...
const styles = {
  ...
  resendLink: { background: 'none', border: 'none', color: '#155DFC', fontWeight: 500, fontSize: '15px', cursor: 'pointer', padding: 0 },
};
